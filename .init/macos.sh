...
defaults read > "${HOME}/defaults.pre.txt"
...
# loop all network intefaces/services and disable ipv6 and proxies
mapfile -t services < <(networksetup -listallnetworkservices | grep -v "An asterisk")
...
echo yes | sudo systemsetup -setremoteappleevents Off >/dev/null
...
defaults write com.apple.screencapture location -string "${CURRENT_USER}/Desktop"
...
defaults write com.apple.finder NewWindowTargetPath -string "file://${CURRENT_USER}/"
...
# Use col view in all Finder windows by default
...
# find "${CURRENT_USER}/Library/Application Support/Dock" -name "*-*.db" -maxdepth 1 -delete
...
# turn off indexing for all volumes
sudo mdutil -i off -a
sudo killall mds
sudo mdutil -i off -a
...
# Install the OneDark theme for iTerm
# open -a iTerm "./.init/OneDark.itermcolors"

# Copy prefs
cp ./.init/com.googlecode.iterm2.p* ~/Library/Preferences/
...
defaults write org.m0k.transmission IncompleteDownloadFolder -string "${CURRENT_USER}/Documents/Torrents"
...
# Disabling updates heavily speeds up virtualized macOS because the qcow2 image does not grow out
...
# sudo -u "${CURRENT_USER}" defaults write NSGlobalDomain NSAppSleepDisabled -bool YES
...
# save new settings and show diff
# defaults read > "${HOME}/defaults.post.txt"

# echo "Diff of changes"
# echo

# diff "${HOME}/defaults.pre.txt" "${HOME}/defaults.post.txt"
# echo
